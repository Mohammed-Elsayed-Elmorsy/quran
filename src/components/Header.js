                ...
                <nav className=' hidden lg:block'>
                    <ul className=' flex gap-4 items-center'>
                        {links.map(link => <li key={link.id}>
                            <Link className={` ${link.chilren ? 'dropdown' : ''} relative rounded-md hover:bg-slate-100 transition capitalize flex items-center gap-1 text-[20px]`} to={link.text == '/' ? '/' : '/' + link.text}>
                                {link.text == '/' ? 'home' : link.text}
                                {link.chilren ? <FaAngleRight className='hide' /> : null}
                                <div className='drop'>
                                    {link.chilren ? link.chilren.map(i => <div key={i}>{i}</div>) : null}
                                </div>
                            </Link>
                        </li>)}
                    </ul>
